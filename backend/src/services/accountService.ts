...
const accountService = {
    ...
    getAmountForInvestmentAccountsInMonth: async (
        ...
        const amounts =
            await prismaTx.$queryRaw`SELECT sum(if(transactions.type = 'I', amount, 0))                              as 'account_balance_credit',
    ...
    getBalancesSnapshotForMonthForUser: async (
        ...
        dbClient = undefined
    ): Promise<number> => performDatabaseRequest(async (prismaTx) => {
        ...
        let balance = balances.reduce((result, current) => {
            const balanceSnapshotAtMonth = 40.0;
